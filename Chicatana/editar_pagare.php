...
$stmt = $conn->prepare("SELECT fecha_deuda,cantidad,pagos
                        FROM pagare WHERE idpagare = ?");
$stmt->bind_param("i",$idpagare);
$stmt->execute();
$stmt->store_result();
$stmt->bind_result($fecha_deuda,$cantidad,$pagos);
$stmt->fetch();
$stmt->close();
?>
...
<body>
  <div class="container">
    ...
    <div class="contenido">
      ...
      <form class="formulario" method="POST" action="actualizar_pagare.php">
        <h3>Información del Pago</h3>

        <input type="hidden" id="idpagare" name="idpagare" value="<?= htmlspecialchars($idpagare)?>">

        <label for="fecha">Fecha</label>
        <input type="text" id="fecha" name="fecha" value="<?= htmlspecialchars($fecha_deuda) ?>" required>

        <label for="cantidad">Cantidad</label>
        <input type="text" id="cantidad" name="cantidad" value="<?= htmlspecialchars($cantidad) ?>" required>

        <label for="pagos">Pagos</label>
        <input type="text" id="pagos" name="pagos" value="<?= htmlspecialchars($pagos) ?>" required>
