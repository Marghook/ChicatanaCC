...
$stmt = $conn->prepare("SELECT num_tarjeta,fecha,concepto,cantidad
                        FROM pago WHERE idpago = ?");
$stmt->bind_param("i",$idpago);
$stmt->execute();
$stmt->store_result();
$stmt->bind_result($num_tarjeta,$fecha,$concepto,$cantidad);
$stmt->fetch();
$stmt->close();
?>
...
<body>
  <div class="container">
    ...
    <div class="contenido">
      ...
      <form class="formulario" method="POST" action="actualizar_pago.php">
        <h3>Información del Pago</h3>

        <input type="hidden" id="idpago" name="idpago" value="<?= htmlspecialchars($idpago)?>">

        <label for="fecha">Fecha</label>
        <input type="text" id="fecha" name="fecha" value="<?= htmlspecialchars($fecha) ?>" required>

        <label for="concepto">Concepto</label>
        <input type="text" id="concepto" name="concepto" value="<?= htmlspecialchars($concepto) ?>" required>
